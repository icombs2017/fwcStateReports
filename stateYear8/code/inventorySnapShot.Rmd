...
frag2 <- frag1 %>% 
  ...
  mutate(
    species = if_else(
      species %in% c("AP", "CN", "DL", "MC", "OF", "PC", "PS"),
      species,  # Keep the original value if it's in the list
      " "   # Replace with 'other' if it's not
    )
  ) %>% 
...
frag3 <- frag1 %>% 
 ...
 mutate(
    species = if_else(
      ...
      filter(monthYear %in% c("07/2024", "10/2024")) %>% 
...
fragPlot <- ggplot(frag3, aes(x = monthYear, y = production, fill = species)) +
  geom_bar(stat = "identity") + # Creates the bar chart
  facet_wrap(~site, scales = "free_y") +# Facets by site, allowing different y-axis scales
  ylim(0, 4000)+
  labs(
    x = "Month/Year", 
    y = "Production", 
    title = "Production by Species and Month/Year"
  )

totalFrag <- frag3 %>% 
  ungroup() %>% 
  dplyr::summarize(total = sum(production))

write.csv(frag3, "../data/stateY8production.csv")
