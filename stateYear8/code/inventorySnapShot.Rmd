...
summarized_data2 <- summarized_data1 %>%
  group_by(species, site) %>%
dplyr::summarize(
...
summerlandEx <- summarized_data2 %>% 
    filter(!str_ends(site, "\\[In Situ\\]"))

summerlandExTab <- summerlandEx %>% 
  select(species, site, total_genotypes, total_TBD, total_0_25, total_0_5, total_0_75, total_1, totalQty) %>% 
  flextable() %>%
  ...
  add_footer_row(values = c("Total", "", 
                            ...
                            sum(summerlandEx$TBD, na.rm = TRUE), 
