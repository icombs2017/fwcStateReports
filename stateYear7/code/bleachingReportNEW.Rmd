...
bleachCombo <- read_excel("../data/postBleachMonitoringTESTnew.xlsx", na = "NA")
...
bleachComboTab <- bleachCombo %>% 
  group_by(Deliverable, Location, `Site ID`) %>% 
  dplyr::mutate(totalSurvival = (aliveNow/aliveBefore)*100) %>%
  flextable() %>%
  set_header_labels(Data.set = "Data set") %>% 
  flextable::compose(part = "header", j = "Deliverable", value = as_paragraph("Year")) %>%
  flextable::compose(part = "header", j = "Location", value = as_paragraph("Location")) %>%
  flextable::compose(part = "header", j = "Site ID", value = as_paragraph("Site")) %>%
  flextable::compose(part = "header", j = "species", value = as_paragraph("Species")) %>%
  flextable::compose(part = "header", j = "aliveBefore", value = as_paragraph("Fragments Alive before the Bleaching Event")) %>%
  flextable::compose(part = "header", j = "aliveNow", value = as_paragraph("Fragments Alive after the Bleaching Event")) %>% 
  flextable::compose(part = "header", j = "totalSurvival", value = as_paragraph("Percent Survived")) %>% 
  autofit() %>%
  flextable::font(fontname = "Times New Roman", part = "all") %>%
  fontsize(size = 12, part = "all") %>%
...
bleachComboDoc = read_docx()
bleachComboDoc = body_add_flextable(bleachComboDoc, value = bleachComboTab)
print(bleachComboDoc, target = "../tables/bleachSurvivalTableEXPANDED.docx")
bleachComboTab
